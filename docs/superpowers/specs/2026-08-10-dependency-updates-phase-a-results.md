...
คอลัมน์ "ก่อน" คือ baseline จริงที่ `main` @ `0a5f901` (ก่อนแตะอะไรเลยในเฟสนี้):

| แพ็กเกจ | ก่อน (baseline `0a5f901`) | หลัง | ปิดช่องโหว่ได้ไหม |
|---|---|---|---|
| `postcss` | 8.5.16 (อินสแตนซ์เดียว) | 8.5.26 (อินสแตนซ์เดียว) | ใช่ — 1 high + 1 moderate |
| `nanoid` | 3.3.15 (อินสแตนซ์เดียว) | 3.3.18 (อินสแตนซ์เดียว) | ใช่ — 2 high |
| `brace-expansion` | 2.1.1 | 2.1.4 | ใช่ — 3 รายการ |
| `@radix-ui/react-dialog` | 1.1.18 | 1.1.23 | — |

**หมายเหตุ — สภาพชั่วคราวระหว่างทาง:** หลัง `bun update` (คอมมิต `cdafb33`) เพียงคำสั่งเดียว
`postcss` และ `nanoid` มี **2 อินสแตนซ์ชั่วคราว** อยู่ในทรี (`8.5.16` คู่กับ `8.5.26`,
`3.3.15` คู่กับ `3.3.18`) เพราะ `bun update` ยกเวอร์ชันของ direct dependency แต่ pin เดิม
ใน `overrides`/`resolutions` (`postcss: ^8.5.6`) ยังอนุญาตให้ตัวเก่าอยู่ในทรีคู่กันได้
สภาพนี้ถูกล้างในคอมมิตถัดไป `6c227eb` (ยก pin ให้ตรงกับเวอร์ชันจริง) เหลืออินสแตนซ์เดียว
ก่อนจะพิสูจน์ผ่าน static/browser gate — **ไม่เคยเป็นสภาพที่หลุดออกจากคอมมิตใดคอมมิตหนึ่งค้างอยู่**
ตัวเลข "ก่อน" ในตารางข้างบนใช้ baseline `0a5f901` เสมอ ไม่ใช่สภาพกลางทางนี้ เพื่อให้รอบหน้าเทียบ
กับฐานที่ถูกต้อง
...
4. commit message ของ `cdafb33` เขียนว่า "27 แพ็กเกจ" — **ตัวเลขนี้ถูกต้อง** มัน
   นับ direct dependency ที่เวอร์ชัน resolve เปลี่ยนจริง (ตรงกับ `bun outdated` ก่อนเริ่มงาน)
   ตัวที่ต่างออกไปคือ semver range ใน `package.json` ที่ถูกแก้ ซึ่งมี **42 รายการ** — มากกว่า
   27 เพราะ `bun update` ยัง bump range ของแพ็กเกจที่เขียนกว้างไว้ (เช่น `^19`) ให้แคบลงตรง
   เวอร์ชันที่ resolve ได้จริง แม้เวอร์ชันนั้นจะไม่ขยับเลยก็ตาม — **42 − 15 = 27**: 15 ตัวใน
   42 เป็นการหดช่วงเฉยๆ โดยเวอร์ชันไม่ขยับ (ตัวอย่างจริง: `zod` `^3` → `^3.25.76`, `eslint`
   `^8` → `^8.57.1`) ส่วนอีก 27 ตัวคือของที่อัปเดตจริง **สองตัวเลขนี้วัดคนละสิ่ง — จำนวน
   range string ที่เปลี่ยน ไม่เท่ากับจำนวนแพ็กเกจที่อัป — ไม่ใช่ว่าตัวใดตัวหนึ่งผิด**
   (ตัวอย่างที่เคยยกไว้ผิดในฉบับก่อนหน้าว่า `react`/`react-dom` `^19` → `^19.2.8` "แม้เวอร์ชัน
   ไม่ขยับ" นั้นผิด — สองตัวนี้ขยับจริงจาก 19.2.7 เป็น 19.2.8)
5. `brace-expansion` ปิดช่องโหว่จริง **3 รายการ ไม่ใช่ 2** — ทั้ง spec design (หัวข้อ 2.2/2.3
   และตาราง Step 4 ในคอมมิต 1) และแผน implementation (ตาราง baseline) นับผิดเป็น `high ×2`
   มาตลอด ทั้งที่ GHSA จริงมี 3 รายการ (`GHSA-mh99-v99m-4gvg`, `GHSA-rgw5-rvv9-x895`,
   `GHSA-3jxr-9vmj-r5cp` — ยืนยันจาก `bun audit` รันจริงที่ baseline `0a5f901`) ตัวเลขนี้
   ลงตัวพอดีกับผล audit ก่อน/หลังในหัวข้อ 1 (`postcss` ปิด 1 high + 1 moderate, `nanoid` ปิด
   2 high, `brace-expansion` ปิด 3 high รวม = 7 ตามที่วัดได้จริง) — แก้แล้วทั้งสองเอกสาร
