...
- **ห่อด้วย `@TryCatch`**, ต่างจาก catch แบบ inline ของ `listCluster:703-710` — ที่นั่น `summary`
  เป็นของแถม ล้มแล้วตารางต้องรอด (ฟิลด์เป็น optional บนสาย และ "ไม่มี summary" มีความหมายนิยามไว้แล้ว
  ฝั่ง client) จึงกลืน error แล้ว log ไว้เฉยๆ แทน ที่นี่ summary **คือ** payload ทั้งหมด `@TryCatch`
  จึงแปลง error ที่โยนออกมาให้กลายเป็น error `Result` ซึ่ง `handleResult` แปลงต่อเป็น HTTP error จริง
  แทนที่จะเป็น 200 ที่ว่างเปล่าให้ FE เดา

> **แก้ตอน execute (2026-08-24, ruling PF-3):** ร่างแรกของ spec เขียนว่าเมธอดนี้ "จงใจไม่ใส่
> try/catch" ซึ่งไปไกลเกินเจตนาจริง — สิ่งที่ spec ปฏิเสธคือ**การกลืน error แบบ inline ของ
> `listCluster`** (catch แล้วทิ้งค่า ยังคืน 200 โดยไม่มี summary) ไม่ใช่การจัดการ error ทุกรูปแบบ
> โค้ดที่ shipped ห่อเมธอดนี้ด้วย `@TryCatch` ซึ่งแปลง error ที่โยนออกมาให้กลายเป็น error `Result`
> → HTTP error จริง ตรงกับเจตนาเดิมทุกประการ เพียงแต่ผลคือผู้อ่านที่ไล่ diff กับ spec จะเจอ decorator
> ชื่อ `TryCatch` บนเมธอดที่ spec บอกว่า "ไม่มี" — แก้ถ้อยคำในย่อหน้านี้ให้ตรงกับของจริงที่ deploy
...
- catch → **เก็บตัวเลขเดิมไว้ ไม่ล้างเป็น `null`** — เลขเก่าที่ยัง valid ถูกต้องกว่าจอว่างเปล่าจาก
  ความล้มเหลวชั่วคราวหนึ่งครั้ง แต่การเก็บไว้เฉยๆ โดยไม่มีสัญญาณอะไรเลยจะทำให้ความล้มเหลว
  **มองไม่เห็นอีกต่อไปหลังโหลดสำเร็จครั้งแรก** จึงต้องคู่กับตัวชี้วัดความเก่า (staleness cue) ในแถบ
  เสมอ — ดู ruling PF-4 ด้านล่าง
- **แก้ `FleetCapacity` ไม่ให้ค้างที่ skeleton ตลอดกาล** — ปัจจุบัน `:83` เรนเดอร์
  ...
  เพิ่ม prop `error?: boolean` แล้วเรนเดอร์บรรทัด `Capacity unavailable` แทน (~6 บรรทัด) — ใช้เมื่อ
  ยังไม่เคยมี `summary` เลย; เมื่อมี `summary` เก่าอยู่แล้วให้ขึ้นตัวชี้วัดความเก่าแทนตามข้อด้านบน

> **แก้ตอน execute (2026-08-24, ruling PF-4):** ร่างแรกของ spec เขียนว่า catch ควร `setFleet(null)`
> เหมือน `LicenseCenter` — โค้ดที่ shipped **ไม่** ทำแบบนั้น เก็บ `summary` เดิมไว้แทน เหตุผล: การล้าง
> เป็น `null` ทิ้งข้อมูลที่ยังถูกต้องไปเพราะความล้มเหลวที่อาจเป็นแค่ชั่วคราว (เช่น network blip ตอน
> refresh หลังลบ cluster) แต่การเก็บไว้แบบเงียบๆ ก็สร้างปัญหาใหม่: `FleetCapacity` gate สถานะ error
> ด้วย `error && !summary` เท่านั้น พอมี `summary` เก่าค้างอยู่ ทุกความล้มเหลวถัดจากครั้งแรกจะมองไม่เห็น
> เลย ตัวเลขเก่าจะดูเหมือนตัวเลขที่เพิ่งโหลดสำเร็จตลอดไป จึงแก้โดยคงตัวเลขเดิมไว้ **และ** เพิ่มตัวชี้วัด
> ความเก่าที่มองเห็นได้ (`role="alert"` + ข้อความ + ทำให้ตัวเลขจางลง) เมื่อ `error && summary` พร้อมกัน
> — ดู `src/pages/clusterManagement/FleetCapacity.tsx`
