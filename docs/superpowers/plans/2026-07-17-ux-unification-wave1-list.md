...
- [ ] **Step 3: Fix loading + empty states** — ~~`TableSkeleton columns={4}`~~ **`TableSkeleton columns={columns.length + 1}`** when `loading && rows.length === 0` (⚠️ this step originally prescribed the literal `4`, which was WRONG — `DataTable` auto-prepends the `#` column, so the table renders 5 headers. The final review caught it; always derive the count, never hardcode); an absolute overlay with `role="status"` when `loading && rows.length > 0`, so add/remove refresh keeps the table visible (copy the pattern at `ClusterManagement.tsx:536-559`). Replace the bare `<EmptyState>` (`:213-218`) with:
  ```tsx
  <ListEmptyState
...
> **Correction (post-final-review):** the coverage claim above was wrong — **ClusterManagement also had the P1** (the audit missed it), making it **9 pages, not 8**. It was fixed in the final-review fix round, so all 9 A3 pages now use `<ListEmptyState>`; only ClusterManagement's **P2 polish** stays deferred. Lesson for later waves: an audit score is evidence, not proof — the "reference" page was never re-verified against the contract it defines.
