...
**Architecture:** Every user-visible literal becomes `t('<key>')`. Already-English strings bind byte-identically. For the 40 Thai-only strings English is **authored** and the Thai is kept verbatim — so five test files that assert those strings must move to the authored English, at exactly 34 lines (see Global Constraints).
...
The last command must list **exactly five** files: `SubscriptionTable.test.tsx`, `SubscriptionForm.test.tsx`, `SeatsCard.test.tsx`, `SubscriptionSummary.test.tsx`, `SubscriptionInfoCard.test.tsx`. Any other test file in that list is a defect.

- [ ] **Step 2: Count the test edits.** `git diff` those five files and confirm **34 changed lines** — `SeatsCard.test.tsx` 15, `SubscriptionTable.test.tsx` 9, `SubscriptionSummary.test.tsx` 5, `SubscriptionForm.test.tsx` 3, `SubscriptionInfoCard.test.tsx` 2 — each moving one of the 40 Thai strings to its authored English (or, for `SubscriptionInfoCard.test.tsx`, the raw English enum lines reassigned per Global Constraints), none deleted or weakened, and confirm the four stated exceptions are still present and still Thai.
