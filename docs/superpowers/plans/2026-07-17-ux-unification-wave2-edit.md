...
- **`UserManagement.tsx:789`** hardcoded `TableSkeleton columns={isSuperAdmin ? 10 : 9}` — matches Lesson 3's exact anti-pattern (a derived count hand-typed instead of computed from the real column set); not touched this round, flagged the same way `TenantMigrationManagement.tsx:461` already is above.
- **Five primary-record error banners carry a comment calling themselves "retryable" but aren't** — `ClusterEdit.tsx:166`, `UserEdit.tsx:214`, `ApplicationEdit.tsx:154`, `RoleEdit.tsx:118`, `ReportTemplateEdit.tsx:227` each have a `// ...transient failure keeps the retryable inline banner.` comment, but the banner itself is a plain `<div role="alert">{error}</div>` with no retry button/action — a 5xx on the primary record fetch leaves a blank form recoverable only by a full browser reload. Either the comments are wrong (reword to drop "retryable") or the banners need an actual retry affordance in Wave 3 — this round didn't adjudicate which.
