...
**Deferred (documented):**
- **Systemic <44px touch targets from the shared `Button`/`Input` `h-9` defaults** — app-wide design decision, still open. **In scope here only where the page authored the undersize itself** — notably `BusinessUnitEdit`'s `InlineField` (~32px across ~50 rows, Task 2) and `ApplicationEdit`'s api-name accordion (24–32px on its primary interaction surface, Task 7). **Specifically `ApplicationEdit.tsx`'s api-name chip grid** (`~120` modules, `flex-wrap` chips at `h-7`/28px, `src/pages/ApplicationEdit.tsx` around the api-names map): Task 7 gave each chip a `HIT_SLOP_44` overlay, but a post-hoc review found the row gap (`gap-2`, 8px) was smaller than the overlay's 8px-per-side bleed, so vertically-adjacent wrapped rows' 44px hit zones overlapped — on this permission-granting surface, an overlapping hit area means a tap can register on the wrong chip and grant the wrong API access, a correctness hazard, not just a small target. The chips were reverted to their pre-Task-7 size (`h-7`, `gap-1.5`, no overlay) rather than ship an overlay that lies about safety; a real ≥44px fix (own height/padding, not an overlay) for this specific surface is still open under this same deferral.
...
**Deferred from the final-review round (2026-07-17):**
- **Stale `<Can>`-masking test mocks + untested permission gates** → Wave 3. `ClusterManagement.test.tsx:31-33` mocks `<Can>` to always render children with no auth mock, over 4 real gates (2 of them `clusterId`-scoped) — the exact landmine pattern Fix 5 removed from `BusinessUnitEdit.test.tsx` in this round. `NewsEdit.test.tsx:9` masks its lone `news.update` gate the same way. `UserPlatformEdit` (3 `<Can>` gates) and `PrintTemplateMappingEdit` (1 gate) have no test file at all, so their gates are completely unverified.
- **`DbConnectionView`'s "Reveal password" toggle has no permission gate of its own** (`src/components/DatabaseConnectionSection.tsx:99`, `src/components/DbConnectionView.tsx`) — it only renders inside `BusinessUnitEdit`'s read-only branch (`canEdit === false`), and that branch is reachable by anyone who cleared the route's broad `cluster.update` check (`App.tsx:136-140`, unscoped) but lacks `cluster.update` on *this specific* BU's cluster. Net effect: an admin with `cluster.update` on cluster A and only `cluster.read` on cluster B can view and reveal cluster B's DB password in plaintext. Pre-existing (not introduced by Wave 2); closing it is a read-authorization product decision (does `cluster.read` alone justify seeing decrypted secrets?), not a mechanical fix.
- **`src/hooks/useUnsavedChanges.ts` only hooks `beforeunload`** — it does not intercept in-app React Router navigation (clicking a nav link, breadcrumb, or any in-app route change bypasses it entirely with zero warning), so the A4 contract's "unsaved guard" promise is only half-true on every Edit page that uses it. A real fix needs a router-level navigation blocker (react-router's `useBlocker` or equivalent), not a per-page patch.
- **`src/utils/validation.ts:22-23`** — `validateField` opens with `if (!value) return '';` and then switches on field-name heuristics, so it structurally cannot express required-ness (an empty value always short-circuits to "no error"). Every A4 page therefore hand-rolls its own separate required-check (`validateRequired()` in `BusinessUnitEdit.tsx`, inline checks elsewhere) instead of getting it from the shared util the contract points to. This needs a util-level fix (e.g. a `required` flag threaded through `validateField`), not another per-page pass.
- **`UserManagement.tsx:786`** hardcoded `TableSkeleton columns={isSuperAdmin ? 10 : 9}` — matches Lesson 3's exact anti-pattern (a derived count hand-typed instead of computed from the real column set); not touched this round, flagged the same way `TenantMigrationManagement.tsx:461` already is above.
- **Five primary-record error banners carry a comment calling themselves "retryable" but aren't** — `ClusterEdit.tsx:176`, `UserEdit.tsx:212`, `ApplicationEdit.tsx:163`, `RoleEdit.tsx:118`, `ReportTemplateEdit.tsx:235` each have a `// ...transient failure keeps the retryable inline banner.` comment, but the banner itself is a plain `<div role="alert">{error}</div>` with no retry button/action — a 5xx on the primary record fetch leaves a blank form recoverable only by a full browser reload. Either the comments are wrong (reword to drop "retryable") or the banners need an actual retry affordance in Wave 3 — this round didn't adjudicate which.
