...
### ⛔ ไม่เขียนเทสต์อัตโนมัติในแผนนี้

**นี่คือกฎประจำตัวของเจ้าของ repo และมันเหนือกว่า `superpowers:test-driven-development`**

- **ห้ามสร้างไฟล์ `*.spec.ts` / `*.test.ts` / `*.test.tsx` ใหม่** ทุก task
- ไม่มีขั้นตอน "เขียนเทสต์ที่ล้ม" / "รันให้เห็นว่าล้ม" / "รันให้เห็นว่าผ่าน"
- **แต่ static check ไม่ใช่เทสต์ — ยังต้องรัน:** type-check + lint ทุก task
- **เทสต์ชุดเดิมที่มีอยู่ต้องยังเขียว** ก่อน merge (รันครั้งเดียวตอนจบเฟส ไม่ใช่ทุก task)
- ถ้า subagent ตัวไหนเริ่มเขียนเทสต์ ให้หยุดมันทันที — มันไม่สืบทอดกฎนี้เอง ต้องบอกตรงๆ ในคำสั่ง
