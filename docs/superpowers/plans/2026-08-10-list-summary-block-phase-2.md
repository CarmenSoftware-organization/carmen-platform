...
> ### ⚠️ SUPERSEDED — ห้ามรัน Task 6 นี้
>
> Pass criterion ด้านล่าง ("ใส่ filter แล้วตัวเลขเปลี่ยนตาม … ถ้าตัวเลขไม่เปลี่ยนตาม filter แปลว่า
> `summary` ไม่ได้มาจริง") **กลับหัวแล้วสำหรับห้าหน้านี้** — แถบสรุปทั้งห้า (`/business-units`,
> `/users`, `/roles`, `/applications`, `/news`) นั่งอยู่**เหนือ** filter และอธิบายทั้งเซ็ต ไม่ใช่
> ผลลัพธ์ที่ถูกกรอง `summary` ของ list endpoint (คำนวณจาก `where` เดียวกับตาราง) จึงเป็นแหล่งข้อมูล
> ที่ผิดสำหรับห้าหน้านี้ ไม่ใช่ถูก การรัน Task 6 ตอนนี้ = เอาบั๊กที่แก้ไปแล้วกลับมาใหม่ทั้งห้าหน้า
> พร้อมชุดเทสต์ที่เขียวหลอก (mock ไม่ได้ครอบพฤติกรรม filter) เก็บ Task 6 ไว้เป็นบันทึกประวัติศาสตร์
> — **อย่าลบ** — แต่อย่าทำตามมัน รายละเอียดและทางแก้ที่แท้จริง (เฟส 2 ที่ยังไม่ออกแบบ ให้ backend
> คืนค่ารวมที่ไม่ผูก filter จริง ๆ แทนการอ่าน `summary` ตัวเดิม):
