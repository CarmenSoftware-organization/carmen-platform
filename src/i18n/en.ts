...
export const en = {
  ...
  pages: {
    ...
    platformMigration: {
      ...
      consoleCopied: 'Log copied',
      consoleCopyFailed: 'Could not copy — select the text and copy it yourself',
      consoleAria: 'Live output from the running operation',
      consoleWaiting: 'Waiting for output...',
      seedsTitle: 'Seed data',
      seedsDescription: 'Writes baseline data into the platform database. Idempotent — rows that already exist are skipped.',
      seedsWrites: 'Writes data',
      checksTitle: 'Drift checks',
      checksDescription: 'Compares what the code declares against what the database actually holds. Nothing here writes.',
      opRun: 'Run',
      opRunWithInput: 'Run…',
      opMissing: 'Not in this build',
      opReadonly: 'Read-only',
      opWrites: 'Writes data',
      opSucceeded: 'Finished',
      opFailed: 'Failed (exit {{code}})',
      ...
      ops: {
        ...
        seedPermission: { label: 'App permissions', desc: 'Every permission the application code declares' },
        seedPlatformPermission: { label: 'Platform permissions', desc: 'The platform-level permission catalog' },
        seedRolePermission: { label: 'Role permissions', desc: 'Bindings between app roles and permissions' },
        seedPlatformRole: { label: 'Platform roles', desc: 'The default platform-level roles' },
        seedPlatformRolePermission: { label: 'Platform role permissions', desc: 'Bindings between platform roles and permissions' },
        seedReportTemplateUpload: { label: 'Report templates', desc: 'Uploads the default report templates' },
        seedLicenseFeature: { label: 'Licensable features', desc: 'The licensing feature catalog' },
        seedApplicationApi: { label: 'App API allowlist', desc: 'Which x-app-id may call which API' },
        checkPermission: { label: 'Check app permissions', desc: 'Finds permissions the code declares but the database lacks, and the reverse' },
        checkPlatformPermission: { label: 'Check platform permissions', desc: 'The same, for platform-level permissions' },
        checkPlatformRolePermission: { label: 'Check platform role permissions', desc: 'Finds role-permission bindings that drifted from what is declared' },
        checkEndpointPermission: { label: 'Check endpoint coverage', desc: 'Finds endpoints with no permission guarding them' },
        checkApiSystemPermission: { label: 'Check api-system coverage', desc: 'The same, limited to api-system routes' },
        checkApplicationApi: { label: 'Check app API allowlist', desc: 'Finds allowlist entries the seed file declares but the database lacks, and the reverse' },
      },
      statusTitle: 'Status',
      upToDate: 'Up to date',
      pendingCount: '{{count}} pending',
      ...
      stateUpToDate: 'Every migration in this build has been applied to the platform database.',
      statePending: 'These migrations exist in this build but have not reached the database yet.',
      stateUnknown: 'The database did not report a migration state. Check the raw output below.',
    ...
    tenantMigration: {
      ...
      columnSchema: 'Schema',
      columnPool: 'pool: {{name}}',
      columnPoolCsv: 'Database Pool',
    ...
    userPlatform: {
      ...
      // ── UserPlatformEdit — the per-holder access dossier ──
      reachPlatformWide: 'Reaches the entire platform',
      reachPlatformWideNote: 'Every cluster and every business unit',
      reachClusters: 'Reaches {{count}} cluster(s)',
      reachNone: 'No platform privilege',
      reachNoneNote: 'This user holds no platform role and will not appear in the registry',
      grantProvenanceUnavailable: 'Grant history unavailable',
      grantNoRecord: 'No grant record',
      selfGranted: 'Self-granted',
      selfGrantedTitle: 'This user issued the grant to their own account',
      emailUnverified: 'Email not verified',
      inactiveHolderDetail: 'This account is deactivated but still holds {{count}} platform grant(s).',
      membershipTitle: 'Membership',
      membershipDescription: 'Where this user works in the tenant tree — not platform privilege',
      membershipEmpty: 'Not a member of any cluster or business unit',
      membershipClusters: 'Clusters',
      membershipBusinessUnits: 'Business units',
      membershipNone: 'None',
      addRoleDescription: 'Grant one platform role at one scope',
      scopePlatformHint: 'The role applies across every cluster',
      scopeClusterHint: 'The role applies only inside the chosen cluster',
    ...
    applications: {
      ...
      // ── ไม้บรรทัดรัศมีร่วมกับหน้ารายการ (utils/apiReach) + การไล่ระดับกริยา (isAuthorityAction) ──
      modulesReach: '{{count}} of {{total}} modules',
      authorityCount: '{{count}} can delete or approve',
      authorityLegend: '{{count}} of these delete records or approve in this app\u2019s name',
      authorityTitle: '{{api}} \u2014 deletes a record or advances a workflow',
      untouchedModules: '{{count}} more module never reached',
      untouchedModulesPlural: '{{count}} more modules never reached',
