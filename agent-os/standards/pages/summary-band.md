...
`FleetCapacity` got its dedicated endpoint first; the other five followed in phase 2 of `docs/superpowers/specs/2026-08-24-summary-band-follows-filter-five-pages-design.md`. Before that endpoint existed, the five read a deliberately filter-free `perpage: -1` list request and rolled it up client-side in a `summarize<Entity>()` per page — that path is gone (removed once the endpoints shipped); do not reintroduce it.
